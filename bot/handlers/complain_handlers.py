...
from lexicon.lexicon_ru import (SEND_GEOLOCATION, SELECT_FROM_LIST,
                                YOUR_OBJECT_IS, OBJECT_IN_BASE, THANK_YOU,
                                FINAL, CHOOSE_OPTIONS, DESCRIBE_PROBLEM,
                                GET_PHOTO, THANKS_WE_WILL_CONTACT_YOU,
                                DESCRIBE_PROBLEM_SECOND, OBJECT_NOT_IN_DB)
from states.tgbot_states import Complain, BaseStates, AssessQualityRepair


async def allowed_not_allowed(callback_query: CallbackQuery,
                              state: FSMContext, force_not_allowed=False):
    ...
    if choice == 'allowed':
        try:
            ...
            pass
        await callback_query.message.answer(SEND_GEOLOCATION,
                                            reply_markup=kb.get_geolocation())
    ...
    if choice == 'not_allowed':
        ...
        await callback_query.message.answer(SELECT_FROM_LIST,
...
@dp.callback_query_handler(
    lambda callback_query: callback_query.data.startswith('city_data_page_'),
    state='*')
async def change_city_page(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(text=SELECT_FROM_LIST,
                                           reply_markup=keyboard)


async def select_from_list_pre(callback_query: CallbackQuery,
    ...
    await callback_query.message.answer(text="Выберите улицу:",
                                        reply_markup=keyboard)
...
@dp.callback_query_handler(
    lambda callback_query: callback_query.data.startswith('road_data_page_'),
    state='*')
async def change_page(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(text="Выберите улицу:",
                                           reply_markup=keyboard)
...
async def search_object(message: Message, state: FSMContext):
    ...
    if road_id:
        ...
        await message.answer(OBJECT_NOT_IN_DB.format(city=city,
                                                     road_name=road_name))
...
async def select_from_list(callback_query: CallbackQuery,
                           state: FSMContext, callback_data: dict):
    await bot.delete_message(callback_query.message.chat.id,
                             callback_query.message.message_id)
    await callback_query.message.answer(YOUR_OBJECT_IS.format(
        _object=callback_data['name']), reply_markup=kb.check_repair())
...
async def check_exists_data_base(callback_query: CallbackQuery,
                                 state: FSMContext):
    await bot.delete_message(callback_query.message.chat.id,
                             callback_query.message.message_id)
    data = await state.get_data()
    ...
    if choose == 'check':
        if 'Complain' in await state.get_state():
            ...
            if info[0]:
                await callback_query.message.answer(
                    OBJECT_IN_BASE.format(period=info[1]),
        ...
        await allowed_not_allowed(callback_query,
                                  state, force_not_allowed=True)
...
async def continue_or_stop(callback_query: CallbackQuery, state: FSMContext):
    ...
    if choose_continue_or_stop == 'stop':
        await callback_query.message.answer(THANK_YOU,
...
async def photo_problem(message: Message, state: FSMContext):
    ...
    if interface.send_report(data, interface.reports_url):
        await message.answer(THANKS_WE_WILL_CONTACT_YOU,
                             reply_markup=kb.yes_no_kb())
        await state.set_state(Complain.final_yes_no)
    else:
        await message.answer(
...
async def final_yes_no(callback_query: CallbackQuery, state: FSMContext):
    ...
    if yes_no == 'yes_back_to_start':
        await callback_query.message.answer(CHOOSE_OPTIONS,
                                            reply_markup=kb.choose_options())
        data = await state.get_data()
        username = data['username']
        phone = data['phone']
        await state.finish()
