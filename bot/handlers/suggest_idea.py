...
async def get_describe_idea(message: Message, state: FSMContext):
    ...
    await message.answer('Можете сделать фото',
                         reply_markup=kb.send_foto_or_not())
    await state.set_state(SuggestIdea.send_foto_about_idea_or_not)


async def send_foto_about_idea_or_not(callback_query: CallbackQuery,
                                      state: FSMContext):
    ...
    if callback_query.data == 'no_photo':
        ...
        if interface.send_report(data, interface.suggestions_url):
            await callback_query.message.answer(THANKS_FOR_IDEA,
                                                reply_markup=kb.yes_no_kb())
            await state.set_state(Complain.final_yes_no)
        else:
            await callback_query.message.answer('Произошла ошибка при \
                отправке данных. Пожалуйста, попробуйте еще раз.')
            await state.finish()
...
def register(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(send_foto_about_idea_or_not, state=SuggestIdea.send_foto_about_idea_or_not)
    dp.register_message_handler(send_foto, content_types=[ContentType.PHOTO],
