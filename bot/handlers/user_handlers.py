...
from lexicon.lexicon_ru import (SEND_AUTO_NUMBER, SEND_HAND_NUMBER,
                                START_MESSAGE, CHOOSE_INPUT_TYPE,
                                YOUR_PHONE_NUMBER, CHOOSE_OPTIONS, ALLOWED,
                                DESCRIBE_IDEA, GET_NAME)
from states.tgbot_states import (BaseStates, Complain,
                                 SuggestIdea, AssessQualityRepair)
...
async def process_name(message: Message, state: FSMContext):
    ...
    await message.answer(CHOOSE_INPUT_TYPE,
                         reply_markup=kb.choose_phone_number())

    await state.set_state(BaseStates.phone)


async def ask_for_auto_contact(callback_query: CallbackQuery,
                               state: FSMContext):
    await bot.delete_message(callback_query.message.chat.id,
                             callback_query.message.message_id)
    await callback_query.message.answer(SEND_AUTO_NUMBER,
                                        reply_markup=kb.ask_for_contact())
    await state.set_state(BaseStates.get_phone)


async def ask_for_hand_contact(callback_query: CallbackQuery,
                               state: FSMContext):
    await bot.delete_message(callback_query.message.chat.id,
                             callback_query.message.message_id)
...
async def save_phone_contact(message: Message, state: FSMContext):
    if message.content_type == ContentType.CONTACT:
        ...
        await message.answer(
            YOUR_PHONE_NUMBER.format(phone_number=phone_number_auto_contact),
            reply_markup=kb.send_button())
    else:
        phone_number_text = message.text
        await state.update_data(phone=phone_number_text)
        await message.answer(YOUR_PHONE_NUMBER.format(
            phone_number=phone_number_text), reply_markup=kb.send_button())
...
async def get_choice(callback_query: CallbackQuery, state: FSMContext):
    ...
    if callback_query.data != 'idea':
        await callback_query.message.answer(
            ALLOWED, reply_markup=kb.allowed_not_allowed())
